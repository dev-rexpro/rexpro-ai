...
var _modified;
import { splitAccessPath, stringValue, mergeConfig, isObject, hasOwnProperty, isNumber, isString, writeConfig, logger, Warn, isArray, array, parseSelector as eventSelector, isBoolean as isBoolean$1, identity, parseExpression as parser, isFunction } from "./fL6EIICl.js";
var version$1 = "6.4.3";
var pkg = {
  version: version$1
};
...
function hasProperty(obj, key) {
  return isObject(obj) && hasOwnProperty(obj, key) && obj[key] !== void 0;
}
const ROW = "row";
const COLUMN = "column";
const FACET = "facet";
...
function selectionAsScaleDomainWithoutField(field) {
  return `A "field" or "encoding" must be specified when using a selection as a scale domain. Using "field": ${stringValue(field)}.`;
}
function selectionAsScaleDomainWrongEncodings(encodings, encoding, extent, field) {
  return `${!encodings.length ? "No " : "Multiple "}matching ${stringValue(encoding)} encoding found for selection ${stringValue(extent.param)}. Using "field": ${stringValue(field)}.`;
}
...
function getScaleInvalidDataMode({ markDef, config, scaleChannel, scaleType: scaleType2, isCountAggregate }) {
  ...
  const scaleOutputForInvalid = (_b = (_a = config.scale) == null ? void 0 : _a.invalid) == null ? void 0 : _b[scaleChannel];
...
function getConditionalValueRefForIncludingInvalidValue({ scaleChannel, channelDef, scale, scaleName, markDef, config }) {
  ...
  if (fieldDef && invalidDataMode === "show") {
    const includeAs = ((_a = config.scale.invalid) == null ? void 0 : _a[scaleChannel]) ?? "zero-or-min";
    return {
...
function formatSignalRef({ fieldOrDatumDef, format, formatType, expr, normalizeStack, config }) {
  ...
  if (isFieldOrDatumDefForTimeFormat(fieldOrDatumDef)) {
    ...
    const signal = timeFormatExpression({
      ...
      isUTCScale: isUTCUnit || isScaleFieldDef(fieldOrDatumDef) && ((_a = fieldOrDatumDef.scale) == null ? void 0 : _a.type) === ScaleType.UTC
    });
...
function valueExpr(v, { timeUnit, type: type2, wrapTime, undefinedIfExprNotRequired }) {
  ...
  let isTime2 = unit || type2 === "temporal";
  let expr;
  if (isExprRef(v)) {
    expr = v.expr;
  ...
  if (expr) {
    return wrapTime && isTime2 ? `time(${expr})` : expr;
  }
...
function initConfig(specifiedConfig = {}) {
  ...
  const mergedConfig = mergeConfig({}, duplicate(defaultConfig), font ? fontConfig(font) : {}, color2 ? colorSignalConfig(color2) : {}, fontSize ? fontSizeSignalConfig(fontSize) : {}, restConfig || {});
  if (selection) {
...
function offsetExpr({ timeUnit, field, reverse: reverse2 }) {
  ...
  const expr = `${offsetFn}('${part}', ${accessWithDatumToUnescapedPath(field)}, ${reverse2 ? -step : step})`;
  return expr;
}
  ...
  signals: (model, selCmpt, signals) => {
    ...
    if (selCmpt.project.hasSelectionId) {
      ...
      const values2 = project2.items.map((p) => {
        ...
        return (fieldDef == null ? void 0 : fieldDef.bin) ? `[${datum}[${stringValue(model.vgField(p.channel, {}))}], ${datum}[${stringValue(model.vgField(p.channel, { binSuffix: "end" }))}]]` : `${datum}[${stringValue(p.field)}]`;
      }).join(", ");
...
function assembleUnitSelectionData(model, data) {
  ...
  for (const selCmpt of vals(model.component.selection ?? {})) {
    ...
    const contains2 = [...selectionData, ...data].filter((d) => d.name === selCmpt.name + STORE);
    if (!contains2.length) {
...
const interval = {
  ...
  signals: (model, selCmpt, signals) => {
    ...
    signals.push(...channels.reduce((arr, proj) => arr.concat(channelSignals(model, selCmpt, proj, init == null ? void 0 : init[proj.index])), []));
    if (!model.hasProjection) {
      ...
      const xinit = x ? init == null ? void 0 : init[x.index] : `${centerSg}[0]`;
      const yinit = y ? init == null ? void 0 : init[y.index] : `${centerSg}[1]`;
  ...
  marks: (model, selCmpt, marks) => {
    ...
    const vgStroke = keys(stroke).reduce((def, k) => {
      def[k] = [
        {
          test: [
            x !== void 0 && `${xvname}[0] !== ${xvname}[1]`,
            y !== void 0 && `${yvname}[0] !== ${yvname}[1]`
          ].filter((t) => t).join(" && "),
          value: stroke[k]
        },
        { value: null }
...
function tooltipData(encoding, stack2, config, { reactiveGeom } = {}) {
  ...
  function add2(fDef, channel) {
    ...
    value ?? (value = addLineBreaksToTooltip(fieldDef, formatConfig, expr).signal);
...
function addLineBreaksToTooltip(channelDef, config, expr = "datum") {
  if (isFieldDef(channelDef) && isDiscrete$1(channelDef.type) && !channelDef.timeUnit && !getFormatMixins(channelDef).format && !getFormatMixins(channelDef).formatType) {
...
function description(model) {
  ...
  return {
    description: {
      signal: entries$1(data).filter(([key]) => !key.startsWith("_")).map(([key, value]) => [key, value.replaceAll("\\n", " ")]).map(([key, value], index) => `"${index > 0 ? "; " : ""}${key}: " + (${value})`).join(" + ")
    }
  };
}
...
function positionAndSize(fieldDef, channel, model) {
  ...
  const timeUnitBandPosition = center && offsetType !== "encoding" && isFieldDef(fieldDef) && fieldDef.timeUnit && !encoding[channel2] ? getBandPosition({ fieldDef, markDef, config }) : void 0;
  const bandPosition = timeUnitBandPosition ?? (center ? offsetType === "encoding" ? 0 : 0.5 : isSignalRef(bandSize) ? { signal: `(1-${bandSize})/2` } : isRelativeBandSize(bandSize) ? (1 - bandSize.band) / 2 : 0);
...
const inputBindings = {
  ...
  topLevelSignals: (model, selCmpt, signals) => {
    ...
    const init = (_a = selCmpt.init) == null ? void 0 : _a[0];
...
function unitName(model, { escape: escape2 } = { escape: true }) {
  let name = escape2 ? stringValue(model.name) : model.name;
...
function isTimerSelection(selCmpt) {
  var _a;
  return (_a = selCmpt.events) == null ? void 0 : _a.find((e) => "type" in e && e.type === "timer");
}
  ...
  for (const part of ["labels", "legend", "title", "symbols", "gradient", "entries"]) {
    ...
    if (value !== void 0 && !isEmpty(value)) {
      legendEncode[part] = {
        ...
        update: (selections == null ? void 0 : selections.length) ? { ...value, cursor: { value: "pointer" } } : value
      };
    }
  }
    ...
    for (const s of t.aggregate) {
      ...
      if (op) {
        if (op === "count") {
          ...
          meas[field] ?? (meas[field] = {});
          (_a = meas[field])[op] ?? (_a[op] = /* @__PURE__ */ new Set());
...
class FacetNode extends DataFlowNode {
  ...
  assembleFacetHeaderData(childIndependentFieldsWithStep) {
    ...
    for (const headerChannel of HEADER_CHANNELS) {
      for (const headerType of HEADER_TYPES) {
        const headers = ((_a = layoutHeaders[headerChannel]) == null ? void 0 : _a[headerType]) ?? [];
...
function parseSelectionDomain(model, channel) {
  ...
  const extent = isBinParams(bin) && isParameterExtent(bin.extent) ? bin.extent : void 0;
  if (domain2 || extent) {
...
function getFieldKeyForChannel(model, channel) {
  if (isUnitModel(model)) {
    ...
    if (fd == null ? void 0 : fd.field) {
      return fd.field;
    }
  }
  const childFields = (model.children ?? []).map((child) => getFieldKeyForChannel(child, channel)).filter((f) => !!f);
...
function extractDiscreteValuesFromDomain(domain2) {
  if (isArray(domain2)) {
    const primitives = domain2.filter(isPrimitive);
    return primitives.length > 0 ? primitives : null;
  }
  if (isDataRefUnionedDomain(domain2)) {
    const values2 = [];
    values2.push(...domain2.fields.flatMap((f) => isArray(f) ? f.filter(isPrimitive) : []));
    if (values2.length > 0) {
      return unique(values2, hash);
    }
  }
  return null;
}
function getDiscreteValuesForChannel(model, channel) {
  try {
    ...
    return extractDiscreteValuesFromDomain(domain2);
  } catch {
    return null;
  }
}
...
function setImplicitLegendValues(cmpt, values2, warnMessage) {
  if (values2 && values2.length > 0) {
    const valuesProp = cmpt.getWithExplicit("values");
    if (!(valuesProp == null ? void 0 : valuesProp.explicit)) {
      if (warnMessage) {
        warn(warnMessage);
      }
      cmpt.set("values", values2, false);
    }
  }
}
function domainsExplicitAndEqual(model, channelA, channelB) {
  ...
  if (!((dA == null ? void 0 : dA.explicit) && (dB == null ? void 0 : dB.explicit)))
    return false;
...
function assembleLegends(model) {
  ...
  const legends = vals(legendsByGroup).flat().map((entry) => assembleLegend(entry.cmpt, model.config)).filter((l) => l !== void 0);
  return legends;
}
...
function parseFacetHeader(model, channel) {
  ...
  if (model.channelHasField(channel)) {
    ...
    const labels2 = fieldDef.header !== null ? getFirstDefined((_a = fieldDef.header) == null ? void 0 : _a.labels, config.header.labels, true) : false;
...
function getMarkGroup(model, opt = { fromPrefix: "" }) {
  ...
  s.on !== "pointerover")) {
    (_a = model.markDef).cursor ?? (_a.cursor = "pointer");
  }
...
class UnitModel extends ModelWithField {
  ...
  alignStackOrderWithColorDomain() {
    var _a;
    const { color: color2, fill, order, xOffset, yOffset } = this.encoding;
    const colorField = fill || color2;
    const colorEncoding = isFieldDef(colorField) ? colorField : void 0;
    const field = colorEncoding == null ? void 0 : colorEncoding.field;
    const scale = colorEncoding == null ? void 0 : colorEncoding.scale;
    const colorEncodingType = colorEncoding == null ? void 0 : colorEncoding.type;
    const domain2 = scale == null ? void 0 : scale.domain;
    const offset = xOffset || yOffset;
    const offsetEncoding = isFieldDef(offset) ? offset : void 0;
    const orderFieldName = `_${field}_sort_index`;
    if (!order && Array.isArray(domain2) && typeof field === "string" && colorEncodingType === "nominal") {
      if (offsetEncoding && !offsetEncoding.sort) {
        offsetEncoding.sort = domain2;
      } else {
        if (!this.stack) {
          return;
        }
        const orderExpression = `indexof(${stringValue(domain2)}, datum['${field}'])`;
        const sort = ((_a = this.markDef) == null ? void 0 : _a.orient) === "horizontal" ? "ascending" : "descending";
