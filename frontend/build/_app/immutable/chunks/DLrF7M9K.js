...
const createPseudoHideStyles = (body, cspNonce2) => {
  createStyles(body, `.${PSEUDO_HIDE_ELEMENT_CLASS_BEFORE}${PSEUDO_BEFORE}${PSEUDO_HIDE_ELEMENT_STYLE}
         .${PSEUDO_HIDE_ELEMENT_CLASS_AFTER}${PSEUDO_AFTER}${PSEUDO_HIDE_ELEMENT_STYLE}`, cspNonce2);
};
const createStyles = (body, styles, cspNonce2) => {
  const document2 = body.ownerDocument;
  if (document2) {
    const style = document2.createElement("style");
    style.textContent = styles;
    if (cspNonce2) {
      style.nonce = cspNonce2;
    }
...
class CacheStorage {
  ...
  static setContext(window2) {
    CacheStorage._link = window2.document.createElement("a");
    CacheStorage._origin = CacheStorage.getOrigin(window2.location.href);
  }
}
...
class CanvasRenderer extends Renderer {
  ...
  truncateTextWithEllipsis(text, maxWidth, letterSpacing2) {
    ...
    if (letterSpacing2 === 0) {
      ...
      for (const letter of letters) {
        const letterWidth = this.ctx.measureText(letter).width + letterSpacing2;
            ...
            switch (paintOrderLayer) {
              case 0:
                ...
                if (styles.letterSpacing === 0) {
                  ...
                  const letters = segmentGraphemes(truncatedText2);
                  letters.reduce((left, letter) => {
  ...
  async renderNodeContent(paint) {
    ...
    if (isTextInputElement(container) && container.value.length) {
      ...
      this.ctx.clip();
      this.renderTextWithLetterSpacing(new TextBounds(container.value, textBounds2), styles.letterSpacing, baseline);
      this.ctx.restore();
      this.ctx.textBaseline = "alphabetic";
      this.ctx.textAlign = "left";
    }
  ...
  debug(...args) {
    if (this.enabled) {
      if (typeof window !== "undefined" && window.console && typeof console.debug === "function") {
        /* @__PURE__ */ console.debug(this.id, `${this.getTime()}ms`, ...args);
      } else {
        this.info(...args);
      }
    }
  ...
  error(...args) {
    if (this.enabled) {
      if (typeof window !== "undefined" && window.console && typeof console.error === "function") {
        /* @__PURE__ */ console.error(this.id, `${this.getTime()}ms`, ...args);
      } else {
        this.info(...args);
      }
    }
