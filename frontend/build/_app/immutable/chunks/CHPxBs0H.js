import "./DKem_M_z.js";
import "./DwLkIEu4.js";
import { K as block, aY as teardown, I as hydrating, J as hydrate_next, S as hydrate_node, bK as ELEMENT_NODE, M as EFFECT_TRANSPARENT, aO as create_element, aK as assign_nodes, bL as is_raw_text_element, aj as get_first_child, ae as create_text, Q as set_hydrating, P as set_hydrate_node, W as active_effect, bM as set_should_intro, aP as NAMESPACE_SVG, p as push, j as onDestroy, l as legacy_pre_effect, n as legacy_pre_effect_reset, h as first_child, a as append, b as pop, k as get, m as deep_read_state, z as sibling, f as from_html, w as set, q as mutable_source, y as event, g as comment } from "./B-Kw_l3V.js";
...
function element(node, get_tag, is_svg, render_fn, get_namespace, location) {
  ...
  block(() => {
    ...
    var ns = next_tag === "svg" ? NAMESPACE_SVG : void 0;
    if (next_tag === null) {
...
function Tooltip($$anchor, $$props) {
  push($$props, false);
  let elementId = prop($$props, "elementId", 8, "");
  let as = prop($$props, "as", 8, "div");
  let className = prop($$props, "className", 8, "flex");
  let placement = prop($$props, "placement", 8, "top");
  let content = prop($$props, "content", 24, () => `I'm a tooltip!`);
  let touch = prop($$props, "touch", 8, true);
  let theme = prop($$props, "theme", 8, "");
  let offset2 = prop($$props, "offset", 24, () => [0, 4]);
  let allowHTML = prop($$props, "allowHTML", 8, true);
  ...
  let tooltipElement = mutable_source(null);
  let tooltipInstance = mutable_source(null);
  function destroyInstance() {
    if (get(tooltipInstance)) {
      get(tooltipInstance).destroy();
      set(tooltipInstance, null);
    }
  }
  onDestroy(() => {
    destroyInstance();
  ...
  legacy_pre_effect(
    () => (get(tooltipElement), deep_read_state(content()), deep_read_state(elementId()), get(tooltipInstance), deep_read_state(placement()), deep_read_state(allowHTML()), deep_read_state(touch()), deep_read_state(theme()), deep_read_state(offset2()), deep_read_state(interactive()), deep_read_state(tippyOptions())),
    () => {
      if (get(tooltipElement) && (content() || elementId())) {
        let tooltipContent = null;
        if (elementId()) {
          tooltipContent = document.getElementById(elementId());
        } else {
          tooltipContent = purify.sanitize(content());
        }
        if (get(tooltipInstance) && get(tooltipInstance).reference !== get(tooltipElement)) {
          destroyInstance();
        }
        if (get(tooltipInstance)) {
          get(tooltipInstance).setContent(tooltipContent ?? "");
        } else {
          if (content()) {
            set(tooltipInstance, tippy(get(tooltipElement), {
      ...
      } else if (get(tooltipInstance) && content() === "") {
        destroyInstance();
      }
    }
  );
  ...
  element(node, as, false, ($$element, $$anchor2) => {
    bind_this($$element, ($$value) => set(tooltipElement, $$value), () => get(tooltipElement));
    attribute_effect($$element, () => ({ class: className() }));
  ...
  slot(node_2, $$props, "tooltip", {}, null);
