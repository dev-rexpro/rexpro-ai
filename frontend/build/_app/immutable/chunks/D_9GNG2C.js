import { P as Parser, N as NodeSet, a as NodeType, D as DefaultBufferLength, b as NodeProp, T as Tree, I as IterMode } from "./CVo5GJDq.js";
...
class Parse {
  ...
  advance() {
    ...
    if (!newStacks.length) {
      ...
      if (finished) {
        if (verbose)
          /* @__PURE__ */ console.log("Finish with " + this.stackID(finished));
        return this.stackToTree(finished);
      }
      if (this.parser.strict) {
        if (verbose && stopped)
          /* @__PURE__ */ console.log("Stuck with token " + (this.tokens.mainToken ? this.parser.getName(this.tokens.mainToken.value) : "none"));
    ...
    if (this.recovering && stopped) {
      ...
      if (finished) {
        if (verbose)
          /* @__PURE__ */ console.log("Force-finish " + this.stackID(finished));
  ...
  advanceStack(stack, stacks, split) {
    ...
    if (this.fragments) {
      ...
      for (let cached = this.fragments.nodeAt(start); cached; ) {
        ...
        if (match > -1 && cached.length && (!strictCx || (cached.prop(NodeProp.contextHash) || 0) == cxHash)) {
          ...
          if (verbose)
            /* @__PURE__ */ console.log(base + this.stackID(stack) + ` (via reuse of ${parser.getName(cached.type.id)})`);
          return true;
        }
    ...
    if (defaultReduce > 0) {
      ...
      if (verbose)
        /* @__PURE__ */ console.log(base + this.stackID(stack) + ` (via always-reduce ${parser.getName(
          defaultReduce & 65535
          /* Action.ValueMask */
        )})`);
      return true;
    ...
    for (let i = 0; i < actions.length; ) {
      ...
      if (verbose)
        /* @__PURE__ */ console.log(base + this.stackID(localStack) + ` (via ${(action & 65536) == 0 ? "shift" : `reduce of ${parser.getName(
          action & 65535
          /* Action.ValueMask */
  ...
  runRecovery(stacks, tokens, newStacks) {
    ...
    for (let i = 0; i < stacks.length; i++) {
      ...
      if (stack.deadEnd) {
        ...
        stack.restart();
        if (verbose)
          /* @__PURE__ */ console.log(base + this.stackID(stack) + " (restarted)");
      ...
      for (let j = 0; j < 10 && force.forceReduce(); j++) {
        if (verbose)
          /* @__PURE__ */ console.log(forceBase + this.stackID(force) + " (via force-reduce)");
      ...
      for (let insert of stack.recoverByInsert(token)) {
        if (verbose)
          /* @__PURE__ */ console.log(base + this.stackID(insert) + " (via recover-insert)");
      ...
      if (this.stream.end > stack.pos) {
        ...
        if (verbose)
          /* @__PURE__ */ console.log(base + this.stackID(stack) + ` (via recover-delete ${this.parser.getName(token)})`);
        pushStackDedup(stack, newStacks);
