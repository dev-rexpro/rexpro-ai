  ...
  static loggerCallback(level, message) {
    switch (level) {
      ...
      case LogLevel.Verbose:
        /* @__PURE__ */ console.debug(message);
        return;
...
class BrowserCacheManager extends CacheManager {
  ...
  async updateOldEntry(key, correlationId) {
    ...
    if ((isAccessTokenEntity(decryptedData) || isRefreshTokenEntity(decryptedData)) && decryptedData.expiresOn && isTokenExpired(decryptedData.expiresOn, DEFAULT_TOKEN_RENEWAL_OFFSET_SEC)) {
      this.browserStorage.removeItem(key);
      this.performanceClient.incrementFields({ expiredCacheRemovedCount: 1 }, correlationId);
      return null;
    }
...
class PlatformAuthInteractionClient extends BaseInteractionClient {
  ...
  async initializeNativeRequest(request) {
    ...
    const mergedClaims = configClaims && configClaims.length ? addClientCapabilitiesToClaims(claims, configClaims) : claims;
    const validatedRequest = {
      ...remainingProperties,
      claims: mergedClaims,
...
/*! @azure/msal-browser v4.30.0 2026-03-18 */
const clientDataAccountTypeMapping = /* @__PURE__ */ new Map([
  ["e", "AAD"],
  ["m", "MSA"]
]);
...
async function getStandardParameters(config, authority, request, logger, performanceClient) {
  const parameters = getStandardAuthorizeRequestParameters({ ...config.auth, authority }, request, logger, performanceClient);
